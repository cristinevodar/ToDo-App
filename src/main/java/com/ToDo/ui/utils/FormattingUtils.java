...
public class FormattingUtils {
	 ...
	 */
	public static final TemporalField WEEK_OF_YEAR_FIELD = WeekFields.of(TasksConst.APP_LOCALE).weekOfWeekBasedYear();
	...
	public static String getFullMonthName(LocalDate date) {
		return date.getMonth().getDisplayName(TextStyle.FULL, TasksConst.APP_LOCALE);
	}

	public static String formatAsCurrency(int valueInCents) {
		return NumberFormat.getCurrencyInstance(TasksConst.APP_LOCALE).format(BigDecimal.valueOf(valueInCents, 2));
	}

	public static DecimalFormat getUiPriceFormatter() {
		DecimalFormat formatter = new DecimalFormat("#" + DECIMAL_ZERO,
				DecimalFormatSymbols.getInstance(TasksConst.APP_LOCALE));
